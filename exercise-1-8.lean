...
(
  ...
  (
    ...
    _ = (-x + x) + n  := by rw [add_left_neg]
    _ = -x + (x + n)  := by rw [add_assoc]
    _ = -x + x        := by rw [hc]
  ...
  have : (1:ℕ) ≤ 0 := Int.ofNat_le.mp this
  Nat.not_succ_le_zero 0 this
)

theorem helper_fact (a : ℤ) (b : ℤ) (n : ℤ) (x : ℤ) (y: ℤ)
...
theorem obvious_fact_2 (c : ℤ): c + c = 2 * c :=
calc
c + c = 1 * c + 1 * c := by rw [one_mul]
    _ = (1 + 1) * c   := by rw [right_distrib]
    _ = 2 * c         := rfl

theorem helper_fact_2 (a : ℤ) (b : ℤ) (n : ℤ) (x : ℤ) (y : ℤ)
...
(
  ...
  (
    ...
    have hhh7: a * b + a * b = 2 * (a * b) := obvious_fact_2 (a * b)
  ...
  (
    ...
    (
      ...
      (
        ...
        have h00b: b + 0 ≤ b + x := Int.add_le_add_left x_nneg b
        have h_b_leq_b0: b ≤ b + 0 := by simp [*] at *
        have h_b_leq_bx: b ≤ b + x := le_trans h_b_leq_b0 h00b
        have hhhh: b ≤ x + b := by {
          simp [*] at *
          exact Int.le_add_of_nonneg_left x_nneg
        }
        show 0 ≤ x + b from le_trans h0b hhhh
      )
      have hya: y - a ≥ 0 := by {
        simp [*] at *
        exact Int.sub_nonneg_of_le h_ygeqa
      }
      have h_different: x + b ≠ x ∨ y - a ≠ y :=
