...
example : nontrivial_power' [0,0,0,0] := by exists 0; exists c'

def primitive' (x : List (Fin 2)) := ¬ nontrivial_power' x

theorem short_primitive' (x: List (Fin 2)) (h: x.length ≤ 1) : primitive' x := by
    intro h_contra
...
theorem primitive_words_of_length_two_or_more (k:ℕ) :
  ∃ x : List (Fin 2), x.length = k.succ.succ ∧ primitive' x := by
...
lemma succ_succ {n:ℕ}(h:n≠0 )(hn:n≠1 ): ∃ k:ℕ, n = k.succ.succ := by
