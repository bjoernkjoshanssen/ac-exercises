...
theorem move_slowly' {A : Type} {k : ℕ} {w : Fin (2 * k + 1) → A}
    {p : Fin (2 * (k + 1)) → Fin (k + 1)}
    {final : Fin (n+1)}
    (h : accepts_path (khδ w) 0 final p) -- generalize to any final state
    (s : Fin (2 * k + 1)) :
...
theorem kayleighBound' {A : Type} {k : ℕ} {w : Fin (2 * k + 1) → A}
  ...
  exact @Fin.induction (2*k+1) (fun n => (p n).1 ≤ n.1)
    (by simp;rw [h.1];simp) (by
      intro n ih
      calc
      _ ≤ (p (n.castSucc)).1 + 1 := @move_slowly' n A k w p 0 h n
  ...
  · obtain ⟨t,ht⟩ := H
    ...
    · by_cases hh : s.1 = k
      ...
      have h₀':= @id_of_slow' k (fun x => p ⟨x.1,by have := x.2;linarith⟩)
        ...
        (by
          intro s;simp
          exact @move_slowly' k A k w p 0 h (s.castLT (by omega))
          -- @move_slowly' n A k w p 0 h n
        ) ⟨s, by omega⟩
      simp at this
      simp at h₀'
      rw [h₀']
