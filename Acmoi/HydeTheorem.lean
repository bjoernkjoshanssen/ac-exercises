...
/-- The NFA generated by a labeled path. -/
def δ_of_path {A : Type*} {q n : ℕ} --(hk : k ≥ 1)
    (w : Fin n → A) (p : Fin (n+1) → Fin q) : A → Fin q → Set (Fin q) :=
  -- -- is s reachable in one step from r reading b?
  fun b r => {s | ∃ t : Fin n, p t = r ∧ p t.succ = s ∧ w t = b}

/-- Kayleigh Hyde's "Kayleigh graph" transition function δ. -/
def khδ' {A : Type*} {k : ℕ} --(hk : k ≥ 1)
  (w : Fin (2*k+1) → A) : A → Fin (k+1) → Set (Fin (k+1)) := by
  let p : Fin (2*k+1+1) → Fin (k+1) :=
    fun t : Fin (2*k+1+1) => dite (t.1 < k + 1) (⟨t.1, .⟩) (⟨2*k+1-t.1, flipCast .⟩)
  exact @δ_of_path A (k+1) (2*k+1) w p
...
theorem move_slowly {A : Type*} {k : ℕ} {w : Fin (2 * k + 1) → A}
  ...
  simp only [mul_eq, dite_eq_ite, Fin.natCast_eq_last] at h
  have := h.2.2 s
  obtain ⟨a,ha⟩ := this
...
theorem hyde_accepts' {A : Type*} {k : ℕ}  (w : Fin (2*k+1) → A) :
  ...
  · simp
  · constructor
    · simp only [Fin.val_last, add_lt_add_iff_right, Nat.sub_self, Fin.zero_eta, dite_eq_right_iff]
      intro h
      omega
    · simp only [Fin.coe_castSucc, Fin.val_succ, add_lt_add_iff_right, reduceSubDiff]
      intro i
      by_cases h₀ : i.1 < k
      · rw [dif_pos h₀]
        have : i.1 < k + 1 := by omega
        rw [dif_pos this]
        unfold khδ' δ_of_path
        use i
        aesop
      · rw [dif_neg h₀]
        unfold khδ' δ_of_path
