...
theorem sub_succ_add_one {n b : ℕ} (hn: n ≥ b.succ) : n - Nat.succ (b) + 1 = n - b
  := by {
  have h₂: n ≥ b := by exact Nat.le_of_lt hn
  have h₃: n - b ≥ 1 := (le_tsub_iff_left h₂).mpr hn
  rw [Nat.sub_succ']; exact Nat.sub_add_cancel h₃
  }
...
theorem fin2cases (i : Fin 2) : i = 0 ∨ i = 1 := by
  ...
  right
  rfl
def moveOne (i s : Fin 2 → ℕ):
  (Nat.succ (Matrix.dotProduct i s)) =
             (i 0 * s 0 + 1 + i 1 * s 1)
      := by {
        have : (i 0 * s 0 + 1 + i 1 * s 1) =
               (i 0 * s 0 + i 1 * s 1) + 1 := by ring
        rw [this]
        rfl
      }
