...
theorem lemma_1_52 (r x y:ℤ)(hr:0<r)(hy:0 ≤ y)(hx:0 ≤ x) :
  ...
  have hrn: r ≠ 0 := by exact Int.ne_of_gt hr --norm_num.ne_zero_of_pos r hr
  ...
  have h1: 2 * r^2 = (x+y)*r+(y+1) → (x,y) = (r,r-1) :=
    ...
    have hz3: 0  = y+1 - r * ((y+1)/r) := calc
              0 = ((2*r)*r) % r := by exact (Int.mul_emod_left (2 * r) r).symm -- (Int.mul_mod_left (2*r) r).symm
            _ = (2 * r^2) % r :=  by ring_nf
            _ = ((x+y)*r+(y+1)) % r := by rw [h]
            _ = ((y+1)+(x+y)*r) % r := by ring_nf
            _ = (y+1)%r := Int.add_mul_emod_self
            _ = y+1 - r * ((y+1)/r) := by exact Int.emod_def (y + 1) r --Int.mod_def (y+1) r
    ...
    have hQ: 0 < (y+1)/r := by exact Int.ediv_pos_of_pos_of_dvd hyy hltr this
