...
\chapter{Nondeterminism and overlap-free words}\label{chap:EJC}
	...
	\section{Introduction}
		...
		\begin{proof}
			\leanok
			Let $n=2m+1$ be odd and let $K_m$ be a Kayleigh graph, \Cref{fig:hyde}.
			For a proof by induction it is convenient to label the states in the opposite order of the usual:
			...
			Then we just observe that there are no walks of odd length $<n$ accepted, and there is exactly one walk of odd length $n$
			accepted, and these two observations persist inductively.

			For a number perspective, let $W_{m,k}$ be the number of accepting walks in $K_M$ of length $k$.
			We show by induction on $m$ that $W_{m,2m+1}=1$ and $W_{m,2s+1}=0$ whenever $s<m$.
		...
		\begin{figure}
			...
			\end{tikzpicture}
			\caption{
				A nondeterministic finite automaton, a \emph{Kayleigh graph}\index{Kayleigh graph}, that only accepts one string
